...
{
    LERPPlanningContext::LERPPlanningContext(const std::string& context_name,const std::string& group_name, const robot_model::RobotModelConstPtr& model)
    ...
    bool LERPPlanningContext::solve(planning_interface::MotionPlanDetailedResponse& res)
    {
        ...
        //调用规划算法求解运动规划问题，返回是否求解成功     ？planning_scene ?request_ ?res_msg
        bool lerp_solved = lerp_interface_->solve(planning_scene_,request_,res_msg);//planning_scene_ , request_是别的头文件包含的
        
        // 如果求解成功，就将求解出的完整的轨迹、处理时间等信息放入应答的数据结构中
        if (lerp_solved)
        {
            res.trajectory_.resize(1);
            // 创建一个指针，指向 robot_trajectory::RobotTrajectory
            res.trajectory_[0] = 
                    robot_trajectory::RobotTrajectoryPtr(new robot_trajectory::RobotTrajectory(robot_model_,getGroupName()));
            
            moveit::core::RobotState start_state(robot_model_);
            // res_msg.trajectory_start 中的数据填充到 start_state 中
            robot_state::robotStateMsgToRobotState(res_msg.trajectory_start,start_state);
            
            // 将 res_msg.trajectory[0] 中的数据填充到 res.trajectory_[0] 中。
